...
class SimplicialComplexOperators {
	...
	buildVertexEdgeAdjacencyMatrix(mesh) {
		let T = new Triplet(this.mesh.edges.length, this.mesh.vertices.length);
		...
		let VEmatrix = SparseMatrix.fromTriplet(T);
	...
	buildEdgeFaceAdjacencyMatrix(mesh) {
		let T = new Triplet(this.mesh.faces.length, this.mesh.edges.length);
		...
		let EFmatrix = SparseMatrix.fromTriplet(T);
	 ...
	 */
	star(subset) {
		let myStar = MeshSubset.deepCopy(subset);

		let VEmatrix = this.A0;
		let Vvector = this.buildVertexVector(subset);
		let adjEdges = VEmatrix.timesDense(Vvector);
		...
		let EFmatrix = this.A1;
		let Evector = this.buildEdgeVector(myStar);
		let adjFaces = EFmatrix.timesDense(Evector);
	 ...
	 */
	closure(subset) {
		let myClosure = MeshSubset.deepCopy(subset);
